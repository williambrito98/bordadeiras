<div class="w-100">
    ...
    <div class="image-preview mt-3" id="imagePreview-{{ $id }}">
        ...
        <input type="file" class="form-control-file" name="{{ $name ?? $id }}{{ isset($multiple) ? '[]' : '' }}"
            id="{{ $id }}" accept="{{ $accept }}" data-initial-files="{{ json_encode($initialFiles) }}"
            {{ isset($multiple) ? 'multiple' : '' }} data-url-add="{{ $urlAdd ?? '' }}"
            data-url-delete="{{ $urlDelete ?? '' }}" data-enable-auto-upload="{{ $enableAutoUpload ?? '' }}"
            data-show-input="{{ $showInput ?? '' }}" data-input-value="{{ json_encode($inputValue ?? []) }}" hidden>
